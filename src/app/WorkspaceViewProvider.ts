  ...
  private async handle(webview: vscode.Webview, request: Request): Promise<void> {
    try {
      switch (request.type) {
        ...
        case 'readerAddReadingSeconds': if (isString(request.uri) && typeof request.seconds === 'number') await this.state.addReaderReadingSeconds(request.uri, request.seconds); break;
