...
- `Workspace: Quick Hide` 保留为可从命令面板调用的中性 Dashboard 切换，不再保留默认 `Ctrl+Alt+H` 键位。
- 新增 `Workspace: Cycle Reader Opacity`：当 Reader Webview 聚焦时按 `Ctrl+B` 在 100%、75%、50%、25%、0% 间切换；设置页滑块提供精确的 0%–100% 调整。这个上下文键位不覆盖编辑器中的 VS Code 默认 `Ctrl+B`（切换侧栏）。
...
- Webview 测试：没有 Online Library，目录切换章节、搜索命中、阅读高度/透明度 CSS 变量、滑块与设置消息正确发出。
