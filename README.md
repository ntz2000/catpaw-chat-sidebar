...
- **样式**：Reader 工具栏内可直接调整 0–100% 透明度，工具栏会同步淡化；可勾选 **Hide controls** 隐藏工具栏，并用细小的 `⋯` 恢复。目录保持高对比、不透明，便于阅读。Settings 中可调字号、行距、宽度、高度和 System/Paper/Dim 主题。
...
`npm run package` 会在项目根目录生成 `catpaw-chat-sidebar-0.0.21.vsix`。
